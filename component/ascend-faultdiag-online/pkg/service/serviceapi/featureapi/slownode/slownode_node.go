...
// Package slownode provides some func to process the data in node
package slownode
...
func nodeProcessSlowNodeAlgoCallback(message string) {
	...
	if err := writeAlgoResult(&nodeResult, slowNodeCtx.Job.Namespace); err != nil {
		hwlog.RunLog.Errorf("[FD-OL SLOWNODE]create node slow node algo result configmap failed: %v", err)
	}
}
...
func nodeReportProfiling(slowNodeCtx *sm.SlowNodeContext) error {
	...
	name := fmt.Sprintf("%s-%s-%s", sm.NodeDataProfilingResultPrefix, slowNodeCtx.Job.JobId, nodeIP)
	if err = createOrUpdateCM(
		name, slowNodeCtx.Job.Namespace, sm.NodeDataProfilingResultCMKey, dataBytes); err != nil {
