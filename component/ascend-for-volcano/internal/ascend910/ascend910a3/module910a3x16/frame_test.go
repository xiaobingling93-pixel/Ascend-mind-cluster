...
func TestScoreBestNPUNodes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			scorePatch := gomonkey.ApplyMethod(reflect.TypeOf(new(rescheduling.ReScheduler)), "ScoreBestNPUNodes",
				func(_ *rescheduling.ReScheduler, _ *api.TaskInfo, _ map[string]float64) error { return nil })
			defer scorePatch.Reset()
