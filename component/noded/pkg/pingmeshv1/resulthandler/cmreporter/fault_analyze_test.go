...
/*
Package cmreporter is using for pingmeshv1 result report to configmap
*/
...
func TestHandlePingMeshInfo(t *testing.T) {
	convey.Convey("Test HandlePingMeshInfo with get last fault error", t, func() {
		r := mockReporter()
		patch := gomonkey.ApplyPrivateMethod(reflect.TypeOf(r), "getLastFault",
			func(_ *faultReporter) (*api.PubFaultInfo, error) {
				return nil, errors.New("")
			})
		defer patch.Reset()
		err := r.HandlePingMeshInfo(nil)
		convey.So(err, convey.ShouldNotBeNil)
	})
}

func TestGetLastFault(t *testing.T) {
	r := mockReporter()
	convey.Convey("Test getLastFault", t, func() {
		r.lastFault = &api.PubFaultInfo{}
		_, err := r.getLastFault()
